...
// A chave global continua sendo a fonte quando use_global_key esta ligado; o
// bloco deixou de ser um return direto porque agora checa creditos antes.
assert.match(apiMain, /if record\.use_global_key:/);
assert.match(apiMain, /config = _get_global_ai_config\(record\)/);
